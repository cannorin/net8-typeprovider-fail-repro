...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Repro.DesignTime", "Repro.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Repro.DesignTime", "Repro.Runtime")])
